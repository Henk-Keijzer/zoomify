...
class ZoomifyState extends State<Zoomify> {
  ...
  Future<void> _loadImageProperties() async {
    final response =
        await http.get(Uri.parse('${widget.baseUrl}/ImageProperties.xml'));
                    ...
                    IconButton(
                        onPressed: () => setState(() => _scrollZoom(
                            PointerScrollEvent(
                                ...
                                scrollDelta: Offset(0.0, -51.0)))),
                        icon:
                            Icon(Icons.add_box, color: widget.zoomButtonColor)),
                    IconButton(
                        onPressed: () => setState(() => _scrollZoom(
                            PointerScrollEvent(
                                position:
...
//        Text('Offset: $horOffset, $verOffset')
  ...
  Widget _buildZoomifyImage() {
    if (_imageDataReady) {
      if (_zoomLevel > -1) {
        ...
        start = _horOffset < -(_tileSize * _scaleFactor)
            ? (-_horOffset ~/ (_tileSize * _scaleFactor))
            : 0;
        end = min(cols as int,
            (1 + (_windowWidth - _horOffset) ~/ (_tileSize * _scaleFactor)));
        visibleCols =
            List.generate(end - start, (index) => (index + start).toInt());

        start = _verOffset < -_tileSize * _scaleFactor
            ? (-_verOffset ~/ (_tileSize * _scaleFactor))
            : 0;
        end = min(rows as int,
            (1 + (_windowHeight - _verOffset) ~/ (_tileSize * _scaleFactor)));
        visibleRows =
            List.generate(end - start, (index) => (index + start).toInt());
        ...
        return SizedBox(
            ...
            child: Stack(
              children: List.generate(visibleRows.length * visibleCols.length,
                  (index) {
                final row = index ~/ visibleCols.length;
                final col = (index % visibleCols.length).toInt();
                final tileUrl =
                    _getTileUrl(_zoomLevel, visibleCols[col], visibleRows[row]);
                return Positioned(
                    left: visibleOffset.dx +
                        col * _tileSize.toDouble() * _scaleFactor,
                    top: visibleOffset.dy +
                        row * _tileSize.toDouble() * _scaleFactor,
                    child: Container(
                        alignment: Alignment.topLeft,
                        width: (col == cols - 1
                                ? (width % _tileSize) * _scaleFactor
                                : _tileSize * _scaleFactor) +
                            (widget.showGrid ? 1 : 0),
                        height: (row == rows - 1
                                ? (height % _tileSize) * _scaleFactor
                                : _tileSize * _scaleFactor) +
                            (widget.showGrid ? 1 : 0),
                        ...
                        child: Image(
                            ...
                            image: NetworkImageProvider(tileUrl,
  ...
  void _setInitialImageData() {
    ...
    var zoom = 0;
    while (zoom < _zoomRowCols.length &&
        _zoomRowCols[zoom]['width'] < _windowWidth &&
        _zoomRowCols[zoom]['height'] < _windowHeight) {
      zoom++;
    }
    _zoomLevel = (zoom < _zoomRowCols.length) ? zoom : _zoomRowCols.length - 1;
    _scaleFactor = _calculateScaleFactor(
        _zoomRowCols[_zoomLevel]['width'].toDouble(),
        _zoomRowCols[_zoomLevel]['height'].toDouble(),
        _windowWidth,
        _windowHeight);
    _scaleFactor = _scaleFactor > 1 ? 1 : _scaleFactor;
    _horOffset =
        (_windowWidth - _zoomRowCols[_zoomLevel]['width'] * _scaleFactor) / 2;
    _verOffset =
        (_windowHeight - _zoomRowCols[_zoomLevel]['height'] * _scaleFactor) / 2;
  }
    ...
    if (event is KeyDownEvent || event is KeyRepeatEvent) {
      switch (event.logicalKey.keyLabel) {
        ...
        case 'H':
          _setInitialImageData();
        case '+' || '=':
          _scrollZoom(PointerScrollEvent(
              position: Offset(_windowWidth / 2, _windowHeight / 2),
              scrollDelta: Offset(0.0, -51.0)));
        case '-' || '_':
          _scrollZoom(PointerScrollEvent(
              position: Offset(_windowWidth / 2, _windowHeight / 2),
  ...
  void _scrollZoom(pointerSignal) {
    ...
    if (pointerSignal.scrollDelta.dy > 0) {
      _zoomOut(ScaleUpdateDetails(
          focalPoint: pointerSignal.position,
          scale: pointerSignal.scrollDelta.dy > 50 ? 0.7 : 0.9));
    } else {
      _zoomIn(ScaleUpdateDetails(
          focalPoint: pointerSignal.position,
          scale: pointerSignal.scrollDelta.dy < -50 ? 1.3 : 1.1));
    }
  ...
  void _zoomOut(scaleDetails) {
    ...
    if (oldWidth > _windowWidth || oldHeight > _windowHeight) {
      ...
      _horOffset =
          -((-_horOffset + scaleDetails.focalPoint.dx) * newWidth / oldWidth) +
              scaleDetails.focalPoint.dx;
      _verOffset = -((-_verOffset + scaleDetails.focalPoint.dy) *
              newHeight /
              oldHeight) +
